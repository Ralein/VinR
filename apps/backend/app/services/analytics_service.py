...
async def get_summary(db: AsyncSession, user_id: str) -> dict:
    ...
    """
    # ⚡ Bolt Optimization: Batched 5 independent aggregate queries into a single query
    # with scalar subqueries to prevent N+1 style roundtrips (from 5 down to 1 query) and improve performance.
    query = select(
        select(func.count(Checkin.id)).where(Checkin.user_id == user_id).scalar_subquery().label("total_checkins"),
        select(func.coalesce(func.sum(Streak.total_days_completed), 0)).where(Streak.user_id == user_id).scalar_subquery().label("total_days_completed"),
        ...
        select(func.count(DailyCompletion.id)).join(Streak, DailyCompletion.streak_id == Streak.id).where(Streak.user_id == user_id).scalar_subquery().label("meditations")
    )
    ...
    return {
        ...
        "total_days_completed": row.total_days_completed or 0,
